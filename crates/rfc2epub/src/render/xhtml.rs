...
/// Mutable state threaded through rendering: the diagram theme mode and the
/// figure collector (populated only in [`SvgMode::Card`]).
pub struct Ctx<'a> {
    pub mode: SvgMode,
...
fn render_inline(inline: &Inline, out: &mut String) {
    match inline {
        ...
        Inline::XRef { text, target } => {
            // Internal references are flattened to plain text for now; a later
            // pass can resolve them to in-book anchors across chapter files.
            let _ = target;
            render_inlines(text, out);
        }
    }

...
/// [`parse_markdown`] when the collection is known.
pub fn parse(body: &str, kind: SourceKind, number: Option<u32>) -> Result<Document> {
    match kind {
        ...
        SourceKind::Mediawiki => Err(Error::Unsupported(
            "MediaWiki sources are not yet supported (only Markdown BIPs). \
             Track: BIP MediaWiki parser milestone."
                .into(),
        )),
